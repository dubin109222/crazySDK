...
public struct EIP712TypedData: Codable {
    ...
    public let domain: GenericJSON
    public let message: GenericJSON
}
...
//        let sss = data.bytes
//        let dataByte = Data([0x19, 0x01]).bytes
//        let domainByte = (try encodeData(data: domain, type: "EIP712Domain", version: version)).sha3(.keccak256).ls_hexString
//        let messageByte = (try encodeData(data: message, type: primaryType, version: version)).sha3(.keccak256).ls_hexString
//        LSLog("domainByte:\(domainByte)")
//        LSLog("messageByte:\(messageByte)")
        return data.sha3(.keccak256)
    }
...
//        let dataByte = encoder.data.bytes
        return encoder.data
    }
    ...
    private func makeABIValue(
        ...
        if components.count == 3 {
            switch version {
            case .v3:
                throw ABIError.arrayNotSupported
            case .v4, .latest:
                let components = type.components(separatedBy: CharacterSet(charactersIn: "[]"))
