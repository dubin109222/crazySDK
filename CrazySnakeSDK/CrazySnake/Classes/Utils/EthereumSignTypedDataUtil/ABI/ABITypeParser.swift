...
struct ABITypeParser {
    ...
    private static func baseTypeMatch(from string: String, length: Int = 0) -> ABIType? {
        ...
        case .bool?:
            return .bool
        case .function?:
            return .function(Function(name: "", parameters: []))
        case .bytes?:
            if length == 0 {
                return .dynamicBytes
            }
        ...
        case .string?:
            return .string
        case .tuple?:
            return .tuple([ABIType]())
        default:
            return nil
        }
    }

    public static func parseTypeString(_ string: String) -> ABIType? {
    ...
    public static func recursiveParseType(_ string: String) -> (type: ABIType?, tail: String?) {
    ...
    public static func recursiveParseArray(baseType: ABIType, string: String) -> (type: ABIType?, tail: String?) {
        ...
        if match[0].numberOfRanges >= 2, let exactArrayRange = Range(match[0].range(at: 2), in: string) {
            ...
            type = ABIType.array(baseType, arrayLength)
        } else {
            type = ABIType.array(baseType, 0)
        }
