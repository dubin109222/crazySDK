...
fileprivate let LSHUD_keyWindow = UIApplication.shared.keyWindow
...
public class LSHUD:UIView {
    ...
    private var hud_cornerRadius : CGFloat = LSHUD_CornerRadiusHasImage
    ...
    init(text:String?,type:LSHUDType,delay:TimeInterval) {
        ...
        self.type = type
        if type == .text {
            hud_cornerRadius = LSHUD_CornerRadiusPureText
        }
    ...
    private func config() {
        ...
        self.layer.cornerRadius = hud_cornerRadius
    ...
    private func addSelfToKeyWindow() {
        ...
        DispatchQueue.main.async {
            LSHUD_keyWindow?.addSubview(self)
            self.alpha = 0
            
            self.addConstraint(width: self.selfWidth, height: self.selfHeight) //
            LSHUD_keyWindow?.addConstraint(toCenterX: self, constantx: 0, toCenterY: self, constanty: -50)
        }
